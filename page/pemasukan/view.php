    ...
    if (isset($_SESSION['user'])) {
        ...
        if (isset($_POST['add'])) {
			...
			if (empty($post_nominal) || empty($post_ket)) {
			    ...
			    $pesan = "User $sesi_user telah menambah data pemasukan sebesar Rp ".number_format($post_nominal,0,',','.');
                $insert_data = mysqli_query($db, "INSERT INTO logs (user, ip, browser, os, note, type, date, time) VALUES ('$sesi_user', '$ip', '$browser', '$os', '$pesan', 'Uang Masuk', '$date', '$time')");
				$insert_data = mysqli_query($db, "INSERT INTO data_kas (user, ket, type, nominal, created_at) VALUES ('$sesi_user', '$post_ket', 'Debit', '$post_nominal', '$date $time')");
				if ($insert_data == TRUE) {
					$msg_type = "success";
			...
			if (empty($post_id) || empty($post_nominal) || empty($post_ket)) {
                ...
                $pesan = "User $sesi_user telah mengubah data pemasukan id ".$post_id;
                $update_data = mysqli_query($db, "INSERT INTO logs (user, ip, browser, os, note, type, date, time) VALUES ('$sesi_user', '$ip', '$browser', '$os', '$pesan', 'Edit Uang Masuk', '$date', '$time')");
			...
			if (mysqli_num_rows($cek_id) == 0) {
                ...
                $pesan = "User $sesi_user telah mendelete data pemasukan id ".$post_id;
                $insert_data = mysqli_query($db, "INSERT INTO logs (user, ip, browser, os, note, type, date, time) VALUES ('$sesi_user', '$ip', '$browser', '$os', '$pesan', 'Delete Uang Masuk', '$date', '$time')");
                        ...
                        <div class="col-lg-6 col-7">
                            <nav aria-label="breadcrumb" class="d-none d-md-inline-block md-4">
                                <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">Pemasukan Kas</a></li>
                                </ol>
                            </nav>
                        </div>
                ...
                <div class="col">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="mb-0">Pemasukan Kas</h3>
                        </div>
                            ...
                            <div class="table-responsive py-4">
                                <table class="table table-flush" id="datatable">
                                    ...
                                    <tbody>
                                        <?php
                                        $no = 0;
            							$cek_data = mysqli_query($db, "SELECT * FROM data_kas WHERE type = 'Debit' ORDER BY id ASC");
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="javascript:;" onclick="modal('<?php echo $config['url']; ?>ajax/pemasukan/view?id=<?php echo enc_id($data_show['id']); ?>')" class="btn btn-sm btn-info"><i class="fa fa-eye" title="View"></i></a>
                                                <a href="javascript:;" onclick="modal('<?php echo $config['url']; ?>ajax/pemasukan/edit?id=<?php echo enc_id($data_show['id']); ?>')" class="btn btn-sm btn-primary"><i class="fa fa-edit" title="Edit"></i></a>
                                                <a href="javascript:;" onclick="modal('<?php echo $config['url']; ?>ajax/pemasukan/delete?id=<?php echo enc_id($data_show['id']); ?>')" class="btn btn-sm btn-danger"><i class="fa fa-trash" title="Hapus"></i></a>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addModalTitle">Tambah Pemasukan Kas</h5>
